...
for ii = 1:length(omegax)
    ...
    X_omega_mag_th(ii) = 1;
     X_omega_angle(ii) = 0;
%     
%     X_omega_mag_th(ii) = 1/sqrt(1 + alpha^(2) - ...
%         alpha*2*cos (omegax(ii)));
%     X_omega_angle(ii) = -atan(alpha^(-1)*sin(omegax(ii))...
%         /(1-alpha^(-1)*cos(omegax(ii))));
...
subplot(4,2,2), plot(omegax/pi, X_omega_mag, 'r',...
    omegax/pi, X_omega_mag_th ,'--k');
xlabel("\omega")
ylabel("X(\omega)")


subplot(4,3,2), plot(omegax/pi, X_omega_angle, 'r',...
    omegax/pi, X_omega_angle ,'--k')
xlabel("\omega")
ylabel("X(\omega)")
...
subplot(4,4,3), stem(n,(x_n_recon), 'g');
xlabel("n")
ylabel("x_r[n]")
